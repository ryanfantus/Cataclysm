...
// Mutation-only traits start here
 ...
 MUTATION(PF_COMPOUND_EYES);
  PREREQS (PF_EYEBULGE);
  CANCELS (PF_SPIDER_EYES_MAIN);
  LEADS_TO (PF_MYOPIC);

 MUTATION(PF_SPIDER_EYES_MAIN);
  PREREQS (PF_EYEBULGE);
  CANCELS (PF_COMPOUND_EYES);
  LEADS_TO (PF_SPIDER_EYES_SECONDARY);

 MUTATION(PF_SPIDER_EYES_SECONDARY);
  LEADS_TO (PF_SPIDER_EYES_MAIN);
...
// Bad mutations below this point
 ...
 MUTATION(PF_EYEBULGE);
  ...
  CHANGES_TO (PF_SPIDER_EYES_MAIN);
...
PF_TROGLO3, PF_SLIMY, NULL);
 ...
 case MUTCAT_CEPHALOPOD:
  setvector(ret,
PF_GILLS, PF_COMPOUND_EYES, PF_MOUTH_TENTACLES, PF_SLIT_NOSTRILS, PF_DEFORMED,
PF_THIRST, PF_BEAK, PF_SLIMY, PF_COLDBLOOD, PF_ARM_TENTACLES_8, PF_SHELL,
PF_LEG_TENTACLES, NULL);
  break;
